...
publishing {
    publications.withType<MavenPublication>().configureEach {
        ...
        artifactId = when (pubName) {
            "kotlinMultiplatform" -> "kachecontroller-cache-sqlite"
            else -> "kachecontroller-cache-sqlite-$pubName"
        }
        val javadocJar = tasks.register<Jar>("${pubName}JavadocJar") {
            dependsOn(tasks.dokkaHtmlPartial)
            archiveClassifier.set("javadoc")
            archiveAppendix.set(pubName)
            from("../docs/kachecontroller-cache-sqlite")
        }
        artifact(javadocJar)
        pom {
            name.set("KacheController SQLite Cache")
            description.set("SQLite cache adapter for KacheController")
            url.set("https://funyin.github.io/KacheController/kachecontroller-cache-sqlite/index.html")
            issueManagement {
                system.set("Github")
